...
#  prefix             = path.expanduser("/content/datalab/data/ecco-data/")
...
#  premiumDocsXRowBase    = "/content/datalab/data/ecco_code/preproc/premiumDocsXRow.csv"
#  premiumCorpusXRowBase  = "/content/datalab/data/ecco_code/preproc/premiumCorpusXRow.csv"
...
def printQueryResults(tops, nTop, period, toDisk = 1):
    ...
    df = pd.DataFrame({
        "id"       : tops.idnr,
        "sentence" : tops.sent,
        "previous" : tops.previous,
        "next"     : tops.next,
        "year"     : years,
        "score"    : tops.score,
        "tokenized": tops.tokenSent
    })
...
def readAllPremium(period, ft, stepsize):
...
def shortlistDoc2Vec(model, year, targetTokenized, docs, corpus, totSents, ids, nPerYear):
    ...
    inferred_vector = modelDoc2Vec.infer_vector(targetTokenized, steps = 20)
...
def wmdTransport(model, target, docs, corpus, previous, post, ids, fs, period):
    ...
    for index in range(nDocs):
        ...
        if index  % 50000 == 0:
            ...
            fs.value += (stepsize*50000)
            print("[{0:9d}/{1:9d}] score = {2:5.3f}\t time = {3:5.2f}\t [Fathomed {4:9d} ({5:4.2f})] {6}".format(index, nDocs, bestUB, timer()-start, totPruned, totPruned/index, star)) 
            star = ""
...
def wmdTransportPenalty(model, target, docs, corpus, previous, post, ids, fs, period):
    ...
    for index in range(nDocs):
        ...
        # if not pruned, solve transportation problem
        # add penalty value
        dd = [ [val if val > 0.0001 else 1.1 for val in d[i]] for i in range(nS)]
        #  for i in range(nS):
        #      for j in range(nD):
        #          if dd[i][j] <= 0.0001:
        #              dd[i][j] = 1.1
...
def wmdTransportForbidden(model, target, docs, corpus, previous, post, ids, fs, forbiddenWords, period):
    '''
    This module is called when there are some forbidden words
    '''
...
def runSearch(periods, query, nrTop, nrSentence, fs, modelWord2Vec, docsD2V, corpusD2V,
totSents, idsD2V, forbidden, withPenalty):
    '''
    ...
    # this is need to ensure that jupyter redirects the output to the notebook
    ipy_stdout = sys.stdout

    period = [str(t) for t in periods]

    fs.value = fs.min

    # process list of forbidden words (if any)
    ...
    if targetTokenized == -1:
        ...
        fTarget.write(query+"\n")
        fTarget.write(str(nrTop)+"\n")
        fTarget.write(str(nrSentence)+"\n")
        fTarget.close()
    ...
    if withCplex == "1":
        if len(forbiddenWords)==0:
            if withPenalty:
                tops = wmdTransportPenalty(modelWord2Vec, targetTokenized, docs, corpus,
                previous, post, ids, fs, period)
            else:
                tops = wmdTransport(modelWord2Vec, targetTokenized, docs, corpus,
                previous, post, ids, fs, period)
        else:
            tops = wmdTransportForbidden(modelWord2Vec, targetTokenized, docs, corpus,
            previous, post, ids, fs, forbiddenWords, period)
    else:
...
def loadData(period, ft):
    ...
    print("\t .... Done in {0:5.2f} seconds.\n".format(timer() - start))
    ft.value = ft.max/(2.0*len(period))

    stepsize = (ft.max-ft.value)/len(period)

    sys.stdout = ipy_stdout 
    # read all preprocessed sentences and keep them in memory
    print("[P2]\t Setting up Word2Vec model ...")
    start = timer()
    strPeriod = [str(y) for y in period]
    docsD2V, corpusD2V, totSents, idsD2V = readAllPremium(strPeriod, ft, stepsize)
    sys.stdout = ipy_stdout 
    ...
    sys.stdout = ipy_stdout 
    return modelWord2Vec, docsD2V, corpusD2V, totSents, idsD2V
